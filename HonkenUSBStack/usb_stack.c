...
Influence and inspiration taken f  http://pe.ece.olin.edu/ece/projects.html
*/
...
#ifdef __DEBUG
#include <stdio.h>
#define DPRINTF(...) printf((const far   char *) __VA_ARGS__)
#else
#define DPRINTF(...)
#endif
...
#define USB_STRING(num, size, ...)   const unsigned char CAT(usb_str_desc,num) [] = {2*(size+1), USB_STRING_DESCRIPTOR_TYPE, __VA_ARGS__};
USB_STRINGS
#undef USB_STRING
...
void usb_send_descriptor( void ) {
	...
	if (usb_desc_len) {
		...
		memcpypgm2ram(rbdp->BDADDR, (const far void *) usb_desc_ptr, packet_len);
