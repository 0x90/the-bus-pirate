...
//
// Version:			1.3.0 january 2010
//
// Link:			http://mrmackey.no-ip.org/elektronik/ds30loader/
//
// History:			1.3.0 Added possibility to use custom bootloader placement and size
...
namespace ds30Loader
{
	...
	static public class clsds30Loader
	{
        ...
        private const int iTestDeviceID = 23;
        private const int iTestVerMaj = 2;
        private const int iTestVerMin = 0;
        private const int iTestVerRev = 2;


		//---------------------------------------------------------------------
		// Constants
		//---------------------------------------------------------------------        
        public const string strVersion = "1.3.0";
		...
		static public void Download ( clsDevice pobjDevice, clsSerialPort pobjPort, clsHex pobjHex, clsDownloadSettings pobjSettings, int iTabLevel, ref bool pbResult )
		{
            ...
            clsds30Loader.ResetDevice( pobjPort, pobjSettings, iTabLevel, ref bResetResult );
            ...
            PortOpen( pobjPort, pobjSettings, true, ref bOpenPortResult, iTabLevel );
            ...
            Version verFw = null;
            FindLoader( pobjDevice, pobjPort, pobjSettings, ref verFw, iTabLevel, ref bFindLoaderResult );
            ...
            if ( pobjSettings.customBl == false ) {
                // Placement
                {
			...
			//--------------------------------------------------------------------------
                ...
                iBlPlacementP = pobjSettings.customBlPlacementP;
                iBlSizeP = pobjSettings.customBlSizeP;
            }
			...
			//--------------------------------------------------------------------------
            clsParseSettings objParseSettings = new clsParseSettings( iBlSizeP, pobjDevice, pobjSettings.allowBlOverwrite, pobjSettings.noGoto, iBlPlacementP );
            ...
            if ( pobjDevice.family.name == "PIC18F" ) {
                ...
                if ( 
                    ( verFw.ToString() == "0.9.0" ) ||
                    ( verFw.ToString() == "0.9.2" ) ||
                    ( verFw.ToString() == "1.0.0" ) ||
                    ( verFw.ToString() == "1.0.1" )
                ) {
            ...
            //--------------------------------------------------------------------------
                ...
                // Write versions 0.9.0 - 1.0.3
                if ( 
                    ( verFw.ToString() == "0.9.0" ) ||
                    ( verFw.ToString() == "0.9.1" ) ||
                    ( verFw.ToString() == "1.0.0" ) ||
                    ( verFw.ToString() == "1.0.1" ) ||
                    ( verFw.ToString() == "1.0.2" ) ||
                    ( verFw.ToString() == "1.0.3" ) 
                ) {
            ...
            //--------------------------------------------------------------------------
                ...
                // Versions 2.0.0 - 2.0.3
                    ...
                    ( verFw.ToString() == "2.0.0" ) ||
                    ( verFw.ToString() == "2.0.1" ) ||
                    ( verFw.ToString() == "2.0.2" ) ||
                    ( verFw.ToString() == "2.0.3" )
				) {
            ...
            //--------------------------------------------------------------------------
                ...
                // Write vrsions 0.9.0 - 1.0.3
                if ( 
                    ( verFw.ToString() == "0.9.0" ) ||
                    ( verFw.ToString() == "0.9.1" ) ||
                    ( verFw.ToString() == "0.9.2" ) ||
                    ( verFw.ToString() == "0.9.3" ) ||
                    ( verFw.ToString() == "0.9.4" ) ||
                    ( verFw.ToString() == "1.0.0" ) || 
                    ( verFw.ToString() == "1.0.1" ) ||
                    ( verFw.ToString() == "1.0.2" ) ||
		...
		// FindLoader()
		//---------------------------------------------------------------------
		static public void FindLoader( clsDevice pobjDevice, clsSerialPort pobjPort, clsDownloadSettings pobjSettings, ref Version pverFw, int iTabLevel, ref bool pbResult )
		{
            ...
            //-----------------------------------------------------------------
                ...
                //--------------------------------------------------------------------------                
                    ...
                    iRcvVerMaj = GetResponse( pobjPort, ref bGetResponseResult );
                    if ( bGetResponseResult == false ) {
                        if ( bPortWasOpened == true ) pobjPort.Close();
			            return;
                    } 
                ...
                //--------------------------------------------------------------------------
                    ...
                    iRcvVerMinRev = GetResponse( pobjPort, ref bGetResponseResult );
		            if ( bGetResponseResult == false ) {
                        if ( bPortWasOpened == true ) pobjPort.Close();
			            return;
		            }		    			
		...
		static private int GetDefaultBlPlacementP( clsDevice pobjDevice, Version pverFw, ref bool pbResult )
		{
            ...
            if ( pobjDevice.family.name == "PIC18F" ) {
                ...
                if ( 
                    ( pverFw.ToString() == "0.9.0" ) ||
                    ( pverFw.ToString() == "0.9.1" )
                ){
                    return 4;
                
                // Firmware version 0.9.2 - 1.0.0
                } else if ( 
                    ( pverFw.ToString() == "0.9.2" ) ||
                    ( pverFw.ToString() == "0.9.2" ) ||
                    ( pverFw.ToString() == "1.0.0" ) ||
                    ( pverFw.ToString() == "1.0.1" )
                ){
                    return 5;
                 
                // Firmware version 1.5 - 1.5
                } else if ( 
                    ( pverFw.ToString() == "1.5.0" ) ||
                    ( pverFw.ToString() == "1.5.0" )
                ){
                    return 3;
            ...
            //--------------------------------------------------------------------------
                ...
                // Firmware version 0.9.0 - 1.0.3
                if ( 
                    ( pverFw.ToString() == "0.9.0" ) ||
                    ( pverFw.ToString() == "0.9.1" ) ||
                    ( pverFw.ToString() == "1.0.0" ) ||
                    ( pverFw.ToString() == "1.0.1" ) ||
                    ( pverFw.ToString() == "1.0.2" ) ||
                    ( pverFw.ToString() == "1.0.3" ) 
                ){
                    return 2;
            ...
            //--------------------------------------------------------------------------
                ...
                // Firmware version 2.0.0 - 2.0.2
                    ...
                    ( pverFw.ToString() == "2.0.0" ) ||
                    ( pverFw.ToString() == "2.0.1" ) ||
                    ( pverFw.ToString() == "2.0.2" ) ||
                    ( pverFw.ToString() == "2.0.3" )
                ) {
					return 4;
		...
		// GetResponse()
		//---------------------------------------------------------------------
		static public int GetResponse( clsSerialPort pobjPort, ref bool pbResult )
		{
            int iResponse = -1;
            pbResult = false;
		...
		// PortOpen()
		//---------------------------------------------------------------------
		static private void PortOpen( clsSerialPort pobjPort, clsDownloadSettings pobjSettings, bool pbDoActivation, ref bool pbResult, int piTabLevel )
		{
            pbResult = false;

            if ( pbDoActivation == true ) {
                // Dtr activation
                if ( pobjSettings.activateDtr == true ) {
                    OnDownloading( new clsDownloadingEventArgs(clsDownloadingEventArgs.EventType.info, "Bringing dtr high...", piTabLevel) );  
                    try {
			            pobjPort.objPort.DtrEnable = true;
                    } catch {
            ...
            if ( pobjPort.isOpen == false ) {
                OnDownloading( new clsDownloadingEventArgs(clsDownloadingEventArgs.EventType.error, "Failed to open port", piTabLevel) );  
            }
		...
		static private void ResetDevice( clsSerialPort pobjPort, clsDownloadSettings pobjSettings, int iTabLevel, ref bool pbResult )
		{
            ...
            if ( pobjSettings.resetCommand == true ) {
                ...
                bool bOpenPortResult = false;
                PortOpen( pobjPort, pobjSettings, false, ref bOpenPortResult, iTabLevel );  
                if ( bOpenPortResult == false ) {
                    return;
                }
		    ...
		    //--------------------------------------------------------------------------            
                ...
                PortOpen( pobjPort, pobjSettings, false, ref bPortOpenResult, iTabLevel );
				...
				for ( int iConfig = 0;  iConfig < clsHex30F.iConfigWordsUsedBufferSize; iConfig++ ) {
					if ( pobjHex.bConfigWordUsed[iConfig] == true ) {
						...
						do {
                            ...
                            if ( bProcessWriteResponseResult == false ) {
                                return false;
                            } else if ( clsds30Loader.debugMode == true ) {
                                OnDownloading( new clsDownloadingEventArgs(clsDownloadingEventArgs.EventType.success, "ok", -1) );
                            }
