...
namespace ds30Loader
{
	...
	public class clsHex18F : clsHex
	{
		...
		// Constants
		//---------------------------------------------------------------------
        public const int iDefaultBlPlacementP = 5;  //page from the end
        public const int iDefaultBlSizeP = 5;       //[pages]
        ...
        override public void ParseHexFile( clsParseSettings pobjSettings, int iTabLevel, ref bool pbResult )
		{
            ...
            // Calculate addresses
            //--------------------------------------------------------------------------
            int iBootloaderStartRow = GetBootloaderStartRow( pobjSettings );
            int iBootloaderEndRow = GetBootloaderEndRow( pobjSettings );
			int iBootloaderStartAdress = GetBootloaderStartAdress( pobjSettings );
            ...
            if ( clsds30Loader.debugMode ) {
                ...
                OnHexFileParse( new clsHexFileParseEventArgs(clsHexFileParseEventArgs.EventType.info, "BootloaderStartAddress: 0x" + iBootloaderStartAdress.ToString("X2"), iTabLevel) );
            }
            ...
            if ( pobjSettings.noGoto == false ) {
                ...
                iProgMem[3] = 0xF0/*goto instruction*/ + ((iBootloaderStartAdress & 0x0F0000) >> 16);	//goto instruction & upper address nibble
            }
